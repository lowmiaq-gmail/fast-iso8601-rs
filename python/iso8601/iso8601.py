...
from ._native import _is_iso8601 as _native_is_iso8601
from ._native import _parse_components
...
def parse_date(
    ...
    try:
        if isinstance(datestring, str):
            native_groups = _parse_components(datestring)
            m = native_groups is not None
        else:
    ...
    # Drop any Nones from the regex matches. Native parsing returns the same
    # named captures directly and leaves Python responsible for object fidelity.
    groups: typing.Dict[str, str] = (
        dict(native_groups)
        if native_groups is not None
        else {k: v for k, v in m.groupdict().items() if v is not None}
    )
...
def is_iso8601(datestring: str) -> bool:
    ...
    try:
        if isinstance(datestring, str):
            return _native_is_iso8601(datestring)
        m = ISO8601_REGEX.match(datestring)
        return bool(m)
    except Exception as e:
        raise ParseError(e)
